...
/* ── Density-based default fills (more institutions → darker) ──── */
function defaultFill(abbrev: string): string {
  switch (abbrev) {
    case 'NSW': return '#4b5563' // gray-600 — 49 institutions
    case 'VIC': return '#4b5563' // gray-600 — 34
    case 'QLD': return '#6b7280' // gray-500 — 16
    case 'SA':  return '#6b7280' // gray-500 — 16
    case 'WA':  return '#9ca3af' // gray-400 — 11
    case 'ACT': return '#9ca3af' // gray-400 — 4
    case 'TAS': return '#9ca3af' // gray-400 — 4
    case 'NT':  return '#9ca3af' // gray-400 — 3
    default:    return '#6b7280'
  }
}

export default function AustraliaMap({ selectedState, onStateSelect, stateCounts }: Props) {
  const selectedAbbrev = selectedState ? ABBREV_FROM_NAME[selectedState] ?? null : null

  function handleClick(abbrev: string) {
    const fullName = STATE_NAMES[abbrev]
    if (selectedState === fullName) {
      onStateSelect(null) // toggle off
    } else {
      onStateSelect(fullName)
    }
  }
    ...
    <div className="max-w-lg mx-auto">
      ...
      >
        ...
        {(['WA', 'QLD', 'SA', 'NT', 'NSW', 'VIC', 'TAS', 'ACT'] as const).map((abbrev) => {
          const d = STATE_PATHS[abbrev]
          if (!d) return null
          const isSelected = abbrev === selectedAbbrev
          return (
            <g key={abbrev} className="cursor-pointer" onClick={() => handleClick(abbrev)}>
              <path
                d={d}
                fill={isSelected ? '#4f46e5' : defaultFill(abbrev)}
                stroke="#111827"
                strokeWidth={0.8}
        ...
        {/* Labels */}
        {Object.entries(LABEL_POS).map(([abbrev, pos]) => {
          const fullName = STATE_NAMES[abbrev]
          const count = stateCounts[fullName] ?? 0
          const isSelected = abbrev === selectedAbbrev
          const isSmall = abbrev === 'ACT' || abbrev === 'TAS'
          return (
            <g
              key={`label-${abbrev}`}
      ...
      <div className="text-center mt-2">
        {selectedState ? (
          <p className="text-xs text-gray-500">
            Filtered to{' '}
            <span className="text-indigo-400 font-medium">{selectedState}</span>
            {' '}&middot;{' '}
            <button
